...
# command qsub -v PATH -v LD_LIBRARY_PATH-cwd -S /bin/bash -j y -l arch=*64*
...
my $do_qdel_on_exit = 0;
my $sge_job_id;
$SIG{INT} = sub {
  print "\nJob canceled.\n";
  if ($do_qdel_on_exit == 1) {
...
print "Job has been submitted. Use 'qstat' to track the state.\n";
#if ($array_job == 1)
{
  my $str_logfile = $logfile;
  $str_logfile =~ s/\$SGE_TASK_ID/*/g;
  print "Now running...\n(log files: $str_logfile)\n";
}
  ...
  foreach my $f (@syncfiles) {
    ...
    while (! -f $f) {
      ...
      if (($check_sge_job_ctr < 100 && ($check_sge_job_ctr++ % 10) == 0) ||
        ...
        my @lines = split(/\n/, $output);
        my $qstat_last_line = $lines[@lines-1];
        if ($qstat_last_line =~ m/Job is in error state/) {
          print STDERR "queue.pl: Job $sge_job_id is in error state\n";
          print STDERR `qstat -j $sge_job_id | grep 'error reason'`."\n";
          if ($qdel_on_Eqw == 1) {
            system("qdel $sge_job_id");
          }
          exit(1);
